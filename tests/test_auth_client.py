...
class TestAuth(TestBaseClient):
    ...
    async def test_sign_up(self):
        ...
        name = self.fake_data.name()
        password = self.fake_data.password(length=10, special_chars=True, digits=True, upper_case=True, lower_case=True)
        ...
        # session check
        assert rsp_sign_user.session.user.user_metadata["name"] == name, "user.user_metadata name is not equal"
        assert rsp_sign_user.session.user.email == email, "sign user email is not equal"
        rsp_get_user = await self.client.auth.get_user(jwt=rsp_sign_user.session.access_token)
    ...
    async def test_refresh_session(self):
        ...
        assert rsp.session.refresh_token != rsp_super.session.refresh_token, "refresh_token is equal"
